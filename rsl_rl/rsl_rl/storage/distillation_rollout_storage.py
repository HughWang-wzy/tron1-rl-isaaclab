...
class DistillationRolloutStorage:
    """Rollout storage for distillation training (student-teacher behavior cloning).

    Stores (obs, actions, privileged_actions, dones) from environment rollouts.
    The ``generator()`` method yields per-timestep batches for behavior cloning updates.
    """
    ...
    class Batch:
        """A named batch of rollout data yielded by :meth:`batch_generator`.

        Used by :class:`MultiExpertDistillation` to access fields by name.
        """
    ...
    def __init__(
        ...
        # Core observations stored as TensorDict
        ...
        # For distillation: teacher's actions as supervision targets
    ...
    def generator(self) -> Generator:
        """Yield per-timestep batches as tuples for distillation training.

        Yields:
            (obs, actions, privileged_actions, dones) for each timestep.
        """
        if self.training_type != "distillation":
            raise ValueError("generator() is only available for distillation training.")
    ...
    def batch_generator(self) -> Generator:
        """Yield per-timestep :class:`Batch` objects for multi-expert distillation training.

        Unlike :meth:`generator`, this method yields named :class:`Batch` objects so that
        :class:`MultiExpertDistillation` can access fields by attribute (``batch.observations``,
        ``batch.privileged_actions``, ``batch.dones``).

        Yields:
            :class:`Batch` for each timestep.
        """
        if self.training_type != "distillation":
            raise ValueError("batch_generator() is only available for distillation training.")
    ...
    def mini_batch_generator(self, num_mini_batches: int, num_epochs: int = 8) -> Generator:
        """Yield mini-batches for RL training (feedforward networks)."""
        if self.training_type != "rl":
            raise ValueError("mini_batch_generator() is only available for RL training.")
    ...
    def recurrent_mini_batch_generator(self, num_mini_batches: int, num_epochs: int = 8) -> Generator:
        """Yield mini-batches for RL training (recurrent networks)."""
        if self.training_type != "rl":
            raise ValueError("recurrent_mini_batch_generator() is only available for RL training.")
